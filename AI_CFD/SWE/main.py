...
ckpt_pth_ae = r"D:\Study\AI_CFD\SWE\logs\epoch=664-step=161595.ckpt"
checkpoint_ae = torch.load(ckpt_pth_ae)
ae_model = SWE_AE(input_size = (1, 3, 384, 256))
ae_model.load_state_dict(checkpoint_ae['state_dict'])

ckpt_pth_linear = r"D:\Study\AI_CFD\SWE\logs\latnet\CSVLogger\finaltest\checkpoints\epoch=14437-step=6424910.ckpt"
checkpoint_linear = torch.load(ckpt_pth_linear)
linear_model = ManifoldNavigator(batch_size=32, hidden_shape = 28)
...
xt = torch.cat([ct, torch.tensor(ref_value[0]).unsqueeze(0)], dim=1).float()
sim_out = decoder(xt)
save_vtk(os.path.join(save_pth, "frame0.vti"), sim_out)


for t in range(1, sim_time//dt + 1):
    print(f"Time: {t*10}sec", end='\r')
    xt = torch.cat([ct, torch.tensor(ref_value[t-1]).unsqueeze(0)], dim=1).float()

    dz = linear_model(xt.unsqueeze(0))

    c_t1 = torch.cat([dz, torch.tensor(ref_value[t]).unsqueeze(0)], dim=1).float()
